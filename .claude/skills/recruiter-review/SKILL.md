...
description: Run a cold recruiter screening pass on a cover letter or resume against a job description, via an Opus subagent. Use when the user asks for a "cold read", "recruiter review", or "feedback" on a letter or resume, and as step 4 of the cover-letter pipeline. Writes a sequential feedback_NNN.md (or resume_feedback_NNN.md) to the org folder and reports the verdict + top edit.
user-invocable: true
---
...
+ the artifact under review and **no prior feedback** (a genuine cold read). Source of truth: memory
`feedback_application_review_protocol`.
...
2. **Spawn an Opus subagent** — `Agent`, `subagent_type: general-purpose`, `model: opus`. The prompt
   makes it a senior technical recruiter doing a cold pass, gives the three file paths, and requests
   this structure written to `orgs/<org>/feedback_NNN.md` (sequential; use `resume_feedback_NNN.md` for
   resume reviews):
...
3. **Embed the honesty constraints in the subagent prompt** so its edits match the voice and don't
   ...
   low-flourish. When facts are already user-confirmed (noted in the org's `notes.md`), tell the subagent
...
4. **Report back** the verdict and the single highest-value edit. Application of edits is the caller's
   job (the `/cover-letter` pipeline, or the user) — apply phrase-level edits that strengthen without
   overclaiming, and **skip** edits that need user confirmation (list them as OPEN).
